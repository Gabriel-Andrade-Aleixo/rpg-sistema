...
import { Backpack, BookOpen, Dices, Download, LayoutDashboard, Sparkles, Swords, TrendingUp, X } from 'lucide-react';
...
import { infusionDamage, infusionManaCost, spectralInfusions, useInfusion, useMagicArrow, useSpell } from '../lib/classActions';
...
function ClassActionsPanel({ character, classEntry, onChange }) {
  ...
  return <Panel title="Flechas mágicas" subtitle="Infusões do Arqueiro Espectral" wide>
    {error && <p className="validationError">{error}</p>}
    <section className="classActionBlock"><h4>Flechas mágicas do Arqueiro Espectral</h4><p className="muted">Informe o dano obtido com o arco. A ficha aplica o modificador da infusão, a Cadência e a penalidade de PM por múltiplos ataques.</p><div className="arrowInputs"><label>Dano base do arco<input type="number" min="0" value={arrowDamage} onChange={(event) => setArrowDamage(event.target.value)} /></label><label>Ataques no turno<input type="number" min="1" value={attacksThisTurn} onChange={(event) => setAttacksThisTurn(event.target.value)} /></label></div><div className="infusionGrid">{spectralInfusions.map((infusion) => { const cadence = Number(character.resources?.cadenciaCurrent || 0); const mana = infusionManaCost(infusion, cadence, attacksThisTurn); const damage = infusionDamage(infusion, arrowDamage, cadence); return <article key={infusion.id}><div><strong>Flecha {infusion.name}</strong><span>{mana} PM · {infusion.focusCost} Foco</span></div><p>{infusion.effect}</p><div className="arrowDamage"><span>Dano no acerto</span><strong>{damage.hit}</strong>{infusion.id === 'spectral' && <><span>Dano no erro</span><strong>{damage.miss}</strong></>}</div><button className="primaryButton compactButton" onClick={() => applyInfusion(infusion)}>Usar flecha</button></article>; })}</div></section>
  </Panel>;
}
...
function SpellbookPanel({ character, catalog, classEntry, onChange }) {
  ...
  const available = catalogGroups(catalog).spells.filter((entry) => {
    ...
    return allowedClass && !knownCatalogIds.has(entry.id) && normalize(`${entry.name} ${entry.description} ${(entry.labels || []).map((label) => label.name).join(' ')}`).includes(normalize(query));
  });
  ...
  return <>
    <Panel title="Grimório do personagem" subtitle="Magias organizadas por tópico" wide>
      ...
      {error && <p className="validationError">{error}</p>}
      {normalize(classEntry?.name || '') === 'arqueiro espectral' && <div className="spectralSpellNotice"><div><strong>Flecha Mágica</strong><span>Dano natural: 2d4</span></div><p>Precisão, Impacto, Perfurante, Cinética e Espectral são infusões que aprimoram a mesma Flecha Mágica.</p><div className="arrowInputs"><label>Resultado dos 2d4<input type="number" min="2" max="8" value={arrowDamage} onChange={(event) => setArrowDamage(event.target.value)} /></label><label>Ataques no turno<input type="number" min="1" value={attacksThisTurn} onChange={(event) => setAttacksThisTurn(event.target.value)} /></label></div></div>}
